...
import { XMarkIcon, LinkIcon, CodeBracketIcon } from '@heroicons/react/24/outline';
...
export const ProjectModal = ({ project, isOpen, onClose }: ProjectModalProps) => {
  ...
  const technologiesByCategory = project.technologies.reduce((acc, tech) => {
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            ...
            >
              <Dialog.Panel className="w-full max-w-4xl transform overflow-hidden rounded-3xl bg-white p-8 text-left align-middle shadow-2xl transition-all border border-gray-100">
                  ...
                  <div className="mb-8 flex gap-4">
                    {project.projectUrl && (
                      <a
                        ...
                        className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-bee-yellow to-yellow-400 text-bee-black font-semibold rounded-xl hover:shadow-lg transition-shadow"
                      >
                    ...
                    {project.githubUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-2 px-6 py-3 bg-gray-900 text-white font-semibold rounded-xl hover:bg-gray-800 transition-colors"
                      >
                ...
                <div className="mb-8">
                  <h4 className="text-2xl font-bold text-gray-900 mb-4">Técnicas Utilizadas</h4>
                  <div className="flex flex-wrap gap-2">
                    {project.techniques.map((technique, index) => (
                      <span
                        key={index}
                        className="px-4 py-2 bg-gradient-to-r from-bee-yellow/10 to-yellow-50 rounded-xl text-sm font-medium text-bee-yellow border border-bee-yellow/30"
                      >
                        {technique}
                      </span>
                    ))}
